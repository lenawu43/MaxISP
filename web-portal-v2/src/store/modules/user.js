...
const user = {
  ...
  mutations: {
    ...
    setdataResource(state,data){
      state.initResource=true;   
      let tempdata={};
      data.forEach(item => {
        if(item.type=="image"){   
          let  ip=window.location.hostname;       
          tempdata[item.keyWord]="http://"+ip+":"+config["img_assignation_url"]+item.keyValue; 
        }
        else{
          tempdata[item.keyWord]=item.keyValue; 
        }        
      });
      debugger
      state.dataResource=tempdata;
    }    
  },
