...
const whiteList = ['/login', '/authredirect', '/product-list','/mobile-product-list','/mobile-login','/browser-product-list','/browser-login','/mobile-unbind-user']// no redirect whitelist
import {
  getMenus
} from '@/api/login.js'
...
router.beforeEach((to, from, next) => { 
  ...
  if (to.query.disabled === '1') {
    ...
    if(to.query.iframe){
      sessionStorage.setItem("iframehiddle",true);
    }
    /* next();
    return; */
      ...
      if (to.path === '/login') {
        next({ path: '/' })
          ...
          if(to.query.token){
            ...
            obj.expires = 30;      
            setToken(to.query.token, obj);
            commit('SET_TOKEN',to.query.token);
          }
