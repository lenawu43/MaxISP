...
service.interceptors.response.use(
  response => {
    ...
    //   return Promise.reject('error')
    // }    
    if ((response.status !== 201 && res.status !== 201)&&((response.status !== 200 && res.status !== 200))) {
      Message({
        message: res.message,
        type: 'error',
        duration: 5 * 1000
      ...
      let  returnData= Object.assign({status:response.status},response.data) ;   
      return  returnData;
    }
  },
  error => {
    ...
    let err = response.data.message? response.data.message : '错误的请求！';    
